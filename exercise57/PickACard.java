...
public class PickACard {
  ...
  // Added shuffleDeck() for the study drill
  public static void shuffleDeck(Card[] deck) {
    int positionOne, positionTwo;
    Card tempOne, tempTwo;

    for(int i = 0; i <= 5000; i++) {
      positionOne = (int)(52 * Math.random());

      do {
        positionTwo = (int)(52 * Math.random());
      } while(positionOne == positionTwo);

      tempOne = deck[positionOne];
      tempTwo = deck[positionTwo];

      deck[positionOne] = tempTwo;
      deck[positionTwo] = tempOne;
    }
  }
